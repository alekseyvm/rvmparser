...
void KdTree::R3StaticTree::getPointsWithinRadiusRecurse(std::vector<QueryResult>& result, uint32_t nodeIndex, const Vec3f& origin, float radius)
{
  ...
  if (node.kind == NodeKind::Leaf) {
    ...
    for (auto i = node.leaf.rangeBegin; i < node.leaf.rangeEnd; i++) {
      ...
      if (d2 <= radiusSquared) {
        result.push_back({points[i].ix, d2});
      }
    }
  }
  else {
...
void KdTree::R3StaticTree::getPointsWithinRadius(std::vector<QueryResult>& result, const Vec3f& origin, float radius)
{
  result.resize(0);
...
void KdTree::R3StaticTree::getNearestNeighboursRecurse(std::vector<QueryResult>& result, uint32_t nodeIx, const Vec3f& origin, uint32_t K)
{
  auto & node = nodes[nodeIx];
  if (node.kind == NodeKind::Leaf) {
    ...
    if (uint32_t(result.size()) < K || d*d < result[K - 1].distanceSquared) {
...
void KdTree::R3StaticTree::getNearestNeighbours(std::vector<QueryResult>& result, const Vec3f& origin, uint32_t K)
{
  result.resize(0);
  result.reserve(K);
